...
export default function Viewnotes({route}) {
  ...
  const noteId = route.params.noteKey;
  console.log(noteId);
  ...
  const findNotes = async ()=>{
    const getNotes = await AsyncStorage.getAllKeys('ZNOTES');
    let listOfNotes = await JSON.parse(getNotes) || [];

    const getNote = listOfNotes.filter ((note)=> note.key ==noteId);
    if(!getNote==null) {
      setNotes(getNote)
    }
  }
      ...
      <View style={styles.contentbox}>
        ...
        <View style={styles.notesdata}>
          ...
          <ScrollView style={{ overflow: 'scroll' }} showsVerticalScrollIndicator={false}>
              <Text style={styles.notesheading}>{notes.title}</Text>
              <Text style={styles.notes}>{notes.desc}</Text>
              </ScrollView>
          </View>
